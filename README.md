...
3 pages, all linked together through the same header and footer:

| File | Purpose |
|---|---|
| `index.html` | Homepage — hero, short About, services teaser, testimonials, call-to-action banner |
...
A section that appears more than once (like `[01]` Header or `[14]` Footer) means that exact same CSS is reused on every page it appears on — so if you edit it in `style.css`, it changes everywhere at once.
...
- `js/main.js` — small scripts: mobile menu toggle, services popup, contact form handler.
- `README.md` — this file.

## What's still placeholder (real business info to fill in)

- **Images**: dashed grey boxes labelled "Photo Here or Video Here" mark where real media goes — swap the `<div class="media-placeholder">...</div>` for an `<img>` (or `<video>`) tag. The `images/` folder is currently empty.
...
- Class names in the HTML (`hero`, `card`, etc.) — the CSS and JS are wired to these; renaming them without updating the CSS/JS will break the styling or the interactive bits (menu, popup).
- The `<script src="js/main.js">` tag at the bottom of each page — without it, the mobile menu and services popup stop working.
